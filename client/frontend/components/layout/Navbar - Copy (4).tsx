            ...
            {/* --- LANGUAGE SWITCHER START --- */}
                        <div className="relative h-16 flex items-center">
                            ...
                            <button
                                ...
                                className={`flex items-center gap-2 transition p-2 rounded-lg border border-transparent
                                    ${isLangMenuOpen
                                        ? 'bg-vault-800 text-white border-vault-700'
                                        : 'text-gray-300 hover:text-white hover:bg-vault-800/50'
                                    }`}
                            >
                                {/* DISPLAY CURRENT FLAG HERE */}
                                <div className="w-6 h-4 flex items-center justify-center rounded-sm overflow-hidden shadow-sm ring-1 ring-white/10">
                                    {currentLangObj.flag}
                                </div>

                                <span className="text-xs font-bold uppercase hidden xl:block ml-0.5">
                                    {currentLangObj.code}
                                </span>
                                        ...
                                        <div className="py-1">
                                            {SUPPORTED_LANGUAGES.map((lang) => (
                                                <button
                                                    ...
                                                    className={`w-full text-left px-4 py-2.5 text-sm flex items-center justify-between transition group
                                                        ${language === lang.code
                                                ...
                                                >
                                                    <div className="flex items-center gap-3">
                                                        {/* Flag in Dropdown */}
                                                        <div className="w-6 h-4 rounded-sm overflow-hidden shadow-sm ring-1 ring-white/10 group-hover:ring-white/30 transition-all">
                                                            {lang.flag}
                                                        </div>
                                                        <span>{lang.name}</span>
                                                    </div>
