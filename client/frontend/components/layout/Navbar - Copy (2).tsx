...
import {
    ...
    Languages, Check // <--- Added Icons for Language Switcher
} from 'lucide-react';

// Import the configuration you provided
...
export const Navbar = ({
    ...
    // 1. Get language state and translation function
    ...
    // Sync local state if filters are updated externally
    useEffect(() => {
    ...
    // 2. Find the current language object for the UI (Flag/Code)
        ...
        <header className="h-16 bg-vault-900 border-b border-vault-700 px-6 flex items-center justify-between shadow-lg w-full">
          ...
          <div className="flex items-center gap-6 pl-4">
            ...
            <div className="relative group h-16 flex items-center">
                <button className="flex items-center gap-2 text-gray-300 hover:text-white transition p-2 rounded-lg hover:bg-vault-800/50">
                    <Languages size={20} />
                    <span className="text-xs font-bold uppercase hidden xl:block">{currentLangObj.code}</span>
                </button>

                {/* Dropdown Menu */}
                <div className="absolute top-12 right-0 pt-2 w-48 hidden group-hover:block z-50">
