...
import { Menu, Gamepad2, Search, ShoppingCart, User as UserIcon, Settings, ShoppingBag, LayoutDashboard, LogOut, ChevronDown } from 'lucide-react';
...
export const Navbar = ({
    ...
    // Sync local state if filters are updated externally (e.g. clear filters)
    useEffect(() => {
    ...
    return (
        <header className="h-16 bg-vault-900 border-b border-vault-700 px-6 flex items-center justify-between shadow-lg w-full">
              ...
              <div className="relative group h-16 flex items-center cursor-pointer">
                ...
                <div className="absolute top-full right-0 pt-2 w-60 hidden group-hover:block z-50">
                   <div className="bg-vault-800 border border-vault-700 rounded-xl shadow-2xl overflow-hidden animate-fadeIn">
                      ...
                      <div className="py-2">
                        <button onClick={() => setView('PROFILE')} className="w-full text-left px-4 py-2.5 text-sm hover:bg-vault-700 flex items-center gap-3 text-gray-300 hover:text-white transition"><Settings size={16}/> Profile Settings</button>
                        
                        {user.role !== 'ADMIN' && (
                          <button onClick={() => setView('ORDERS')} className="w-full text-left px-4 py-2.5 text-sm hover:bg-vault-700 flex items-center gap-3 text-gray-300 hover:text-white transition"><ShoppingBag size={16}/> My Orders</button>
                        )}
    
                        {user.role === 'ADMIN' && (
                          <button onClick={() => setView('ADMIN')} className="w-full text-left px-4 py-2.5 text-sm hover:bg-vault-700 flex items-center gap-3 text-vault-accent font-medium bg-vault-accent/5"><LayoutDashboard size={16}/> Admin Dashboard</button>
                        )}
                        <button onClick={() => setView('STORE')} className="w-full text-left px-4 py-2.5 text-sm hover:bg-vault-700 flex items-center gap-3 text-gray-300 hover:text-white transition"><Gamepad2 size={16}/> Browse Store</button>
                      </div>
                      <div className="border-t border-vault-700 py-2 bg-vault-900/30">
                        <button onClick={logout} className="w-full text-left px-4 py-2 text-sm text-red-400 hover:bg-red-900/20 flex items-center gap-3 transition"><LogOut size={16}/> Sign Out</button>
                      </div>
                   </div>
                </div>
              </div>
