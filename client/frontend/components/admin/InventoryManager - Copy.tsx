...
export const InventoryManager = () => {
    ...
    const handleSave = async () => {
        ...
        const result = await BackendService.saveGame(editingGame);
        if (result) {
    ...
    const processFiles = async (files: File[]) => {
        ...
        try {
            ...
            for (const file of files) {
                const url = await BackendService.uploadImage(file);
    ...
    const isLocalFile = (url: string) => {
        // Check if URL contains "api/images" indicating it's hosted by us
        return url && url.includes("/api/images/");
    };
    ...
    return (
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 gap-4">
            ...
            <div className="flex items-center gap-3">
                ...
                {!isArchiveView && (
                    <button
                    onClick={() => { setEditingGame({ images: [], categories: [], quantity: 0, isArchived: false }); setIsModalOpen(true); }}
                    ...
                    <tr key={game.id} className="hover:bg-vault-800 transition-colors">
                        ...
                        {visibleColumns.includes('Actions') && (
                            ...
                            <div className="flex justify-end gap-2">
                                {isArchiveView ? (
                                    <>
                                        <button onClick={() => { setEditingGame(game); setIsModalOpen(true); }} className="p-2 text-blue-400 hover:bg-blue-900/20 rounded transition" title="Edit"><Edit size={16} /></button>
                                        <button
        ...
        {isModalOpen && (
            ...
            <div className="bg-vault-800 p-8 rounded-3xl w-full max-w-4xl border border-vault-700 shadow-2xl relative h-[90vh] overflow-y-auto custom-scrollbar flex flex-col">
                <button onClick={() => setIsModalOpen(false)} className="absolute top-6 right-6 text-gray-500 hover:text-white transition bg-vault-900 p-2 rounded-full"><X size={24}/></button>
                ...
                <div className="flex-1 space-y-6">
                    ...
                    <div>
                        ...
                        {uploadingImg && <span className="block text-xs text-vault-accent animate-pulse mb-2">{t('inv_img_uploading')}</span>}

                        {/* Input Area */}
                        {imageInputMode === 'UPLOAD' ? (
                            <div
                                className={`w-full p-4 bg-vault-950 rounded-xl border-2 border-dashed hover:border-vault-accent transition flex flex-col items-center justify-center gap-4 relative min-h-[160px] ${uploadingImg ? 'border-vault-accent/50 bg-vault-900/50' : 'border-vault-700'}`}
                            ...
                            >
                                ...
                                >
                                    {uploadingImg ? (
                                        <Loader className="animate-spin text-vault-accent mb-2" size={48}/>
                                    ) : (
                                        <ImageIcon size={48} className="mb-2 opacity-50"/>
                                    )}
                                    <p className="font-bold">{uploadingImg ? t('inv_img_uploading') : t('inv_img_drag')}</p>
                                    <p className="text-xs">{!uploadingImg && t('inv_img_click')}</p>
                                ...
                                <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-5 gap-4 w-full">
                                    {editingGame.images.map((img, idx) => {
                                        ...
                                        >
                                            ...
                                            <button
                                                onClick={() => setEditingGame(prev => ({...prev, images: prev.images?.filter((_, i) => i !== idx)}))}
                ...
                <button onClick={() => setIsModalOpen(false)} className="px-6 py-3 text-gray-400 hover:text-white transition font-bold text-sm">{t('inv_btn_cancel')}</button>
                <button onClick={handleSave} disabled={uploadingImg} className="flex items-center gap-2 px-8 py-3 bg-vault-accent text-white font-bold rounded-xl hover:bg-vault-accentHover transition shadow-xl shadow-vault-accent/20 text-sm transform active:scale-95 disabled:opacity-50">
                    <Save size={18}/> {t('inv_btn_save')}
                </button>
                </div>
            </div>
            </div>
