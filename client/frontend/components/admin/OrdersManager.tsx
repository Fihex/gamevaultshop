...
import {
    Filter, Users, Phone, ShoppingBag, ChevronDown, Loader,
    CheckCircle2, Package, Clock
} from 'lucide-react';
...
const StatusDropdown = ({ currentStatus, onUpdate, isLoading, t }: StatusDropdownProps) => {
    ...
    const activeConfig = config[currentStatus] || config[OrderStatus.ORDERED];
        ...
        <div className="relative">
            ...
            <button
                ...
                className={`
                    ...
                    ${isLoading ? 'opacity-50 cursor-not-allowed' : 'cursor-pointer hover:brightness-110'}
                `}
            >
                <ActiveIcon size={14} />
                <span className="text-xs font-bold uppercase tracking-wider">{t(`status_${currentStatus}`)}</span>
                <ChevronDown size={12} className={`transition-transform duration-200 ${isOpen ? 'rotate-180' : ''}`} />
            </button>

            {/* Dropdown Menu */}
            {isOpen && (
                <div className="absolute right-0 top-full mt-2 w-48 bg-vault-950 border border-vault-700 rounded-lg shadow-xl z-20 overflow-hidden animate-in fade-in slide-in-from-top-2 duration-100">
                    {Object.values(OrderStatus).map((status) => {
                        const ItemIcon = config[status].icon;
                        ...
                        return (
                            <button
                                ...
                                className={`
                                    w-full text-left px-3 py-2.5 text-xs font-bold uppercase flex items-center gap-3 transition-colors
                                    ${isSelected ? 'bg-vault-800 text-white' : 'text-gray-400 hover:bg-vault-800 hover:text-gray-200'}
                                `}
                            >
                                <ItemIcon size={14} className={isSelected ? config[status].color : 'text-gray-500'} />
                                {t(`status_${status}`)}
                            </button>
                        );
                    })}
...
export const OrdersManager = () => {
    ...
    return (
        <div>
            ...
            <TopToolbar
                search={search} setSearch={setSearch} setPage={setPage} placeholder={t('admin_orders_search_ph')}
            ...
            <div className="relative min-h-[300px]">
                ...
                <div className="space-y-6">
                    {(data?.content || []).map(order => {
                        ...
                        return (
                        <div key={order.id} className="bg-vault-900 border border-vault-700 rounded-xl shadow-sm hover:shadow-md transition-all hover:border-vault-600">
                            {/* Header */}
                            {/* Added rounded-t-xl to ensure header corners match the container since we removed overflow-hidden */}
                            <div className="bg-vault-950/50 rounded-t-xl p-4 flex flex-wrap gap-4 justify-between items-center border-b border-vault-800">
