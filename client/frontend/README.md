...
### 1. System Setup
Update system and install common tools including the firewall (UFW).
```bash
sudo apt update && sudo apt upgrade -y
sudo apt install postgresql postgresql-contrib nginx certbot python3-certbot-nginx ufw -y
```
...
# Inside SQL Shell:
CREATE DATABASE gamevaultjava;
CREATE USER vaultuser WITH ENCRYPTED PASSWORD 'secure_password';
GRANT ALL PRIVILEGES ON DATABASE gamevaultjava TO vaultuser;
...
2.  **Upload & Build:**
    ...
    **If using Maven:**
    ...
    # Build (This automatically downloads all libraries defined in pom.xml)
    ...
    **If using Gradle:**
    ...
    # Build (This automatically downloads all libraries defined in build.gradle)
    chmod +x gradlew
...
3.  **Create Service:**
    ...
    sudo nano /etc/systemd/system/gamevault-spring-backend.service
...
#### Option B: Go (Golang)
1. **Install Go 1.21:**
   Default Ubuntu repositories often have older versions. We will install Go 1.21 manually.
   
   ```bash
   # 1. Download the Go 1.21 binary
   wget https://go.dev/dl/go1.21.6.linux-amd64.tar.gz

   # 2. Extract it to /usr/local
   sudo rm -rf /usr/local/go && sudo tar -C /usr/local -xzf go1.21.6.linux-amd64.tar.gz

   # 3. Add Go to your PATH (so you can run the 'go' command)
...
2. **Upload & Build:**
   ...
   # Build the binary (This automatically downloads libraries defined in go.mod)
   go mod tidy
...
3. **Create Service:**
   ...
   Environment="DB_PASSWORD=CVyP9RG4pdGtJBNMbYfqGuQqa"
   Environment="DB_NAME=gamevault"
   Environment="JWT_SECRET=qF2Ynxdn6LFwfGjs5jsMwWCYsxunP4zmQbvS5TsD3Fm8ekVMpWWWMQCrx3AoJNycaYRXG4G5vwpenvewutDnZ5ysATny8"
...
2. **Upload & Build:**
   ...
   # Install Libraries (Reads package.json and downloads dependencies)
   npm install
...
> Unlike development (where you run `npm start`), in production you do not run a command to keep the frontend running. Instead, we configure **Nginx** (in the next step) to serve these static files. **Starting Nginx *is* starting the frontend.**
...
server {
    listen 80;
    server_name 144.31.19.175; # e.g., 192.168.1.1
...
Based on your request, I have updated the Sidebar.tsx file to correctly implement the sidebar filters (categories, price) and fixed the truncated code.

Here is the answer to your question about starting and stopping the services:

### How to Start and Stop Services

Since you are using systemd (as per the deployment guide), you manage the services using systemctl.

**1. Backend (Java or Go)**  
The backend runs as a background service named gamevault-backend.

- **Start:** sudo systemctl start gamevault-backend
  
- **Stop:** sudo systemctl stop gamevault-backend
  
- **Restart:** sudo systemctl restart gamevault-backend
  
- **Check Status:** sudo systemctl status gamevault-backend
  

**2. Frontend (React)**  
The frontend is static files served by **Nginx**. You do not "start" the React app like you do in development (e.g., npm start). Instead, you control Nginx.

- **Start:** sudo systemctl start nginx
  
- **Stop:** sudo systemctl stop nginx
  
- **Restart:** sudo systemctl restart nginx (Use this if you change Nginx config)
...
# Part 5: Service Management (How to Start/Stop)

Once deployed, you may need to manually start or stop the services.
...
The frontend is served by Nginx. Starting Nginx effectively "starts" the frontend.
```bash
# Start Nginx
sudo systemctl start nginx

# Restart Nginx (if you changed config)
sudo systemctl restart nginx
```
...
# Part 8: Understanding Library Installation (Dependencies)

You asked about installing libraries for Java, Golang, or Frontend. In modern web development, you do **not** manually download libraries (like `.jar` or `.js` files). Instead, you use **Package Managers**.

When you run the build commands listed in this guide, the system automatically reads a configuration file and downloads exactly what is needed.

### 1. Frontend (Node.js/React)
*   **Config File:** `package.json`
*   **How it works:** This file lists every library your website needs (e.g., React, Tailwind, Lucide Icons).
*   **The Command:** `npm install`
    *   This command reads `package.json` and downloads thousands of small files into a `node_modules` folder.
    *   This happens automatically in Part 1 (Step 4) of this guide.

### 2. Java Backend (Spring Boot)
*   **Config File:** `pom.xml` (Maven) or `build.gradle` (Gradle).
*   **How it works:** This file lists dependencies like Spring Security, PostgreSQL Driver, and JWT tokens.
*   **The Command:** `mvn package` or `./gradlew build`
    *   This command connects to "Maven Central" (a global server of Java libraries) and downloads them to your VPS.
    *   This happens automatically in Part 1 (Step 3) of this guide.

### 3. Go Backend (Golang)
*   **Config File:** `go.mod`
*   **How it works:** This file lists GitHub repositories that contain the code your app needs (e.g., Gin Web Framework, GORM).
*   **The Command:** `go mod tidy` or `go build`
    *   Go checks if you have these libraries. If not, it downloads them securely.
    *   This happens automatically in Part 1 (Step 3 - Option B) of this guide.

---

# Part 9: Creating an Admin User (First Time Setup)

Since the database starts with no users, you must manually promote the first user to `ADMIN` to access the dashboard.

1.  **Create an Account:** Go to your deployed website and register a new user (e.g., username `admin`).
2.  **SSH into VPS:** Connect to your server terminal.
3.  **Open Database Shell:**
    ...
    # For Docker Deployment:
    docker exec -it gamevault-db-1 psql -U vaultuser -d gamevault
    ```
...
5.  **Exit SQL Shell:**
    ...
    \q
    ```
6.  **Verify:** Log out of the website and log back in. You should now see "Admin Dashboard" in the user dropdown menu.
